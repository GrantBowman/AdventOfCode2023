...
with open("input23_demo.txt") as input:
    # print("hello world")
    ...
    while stack:
        ...
        #     test = True
        # no slopes into walls (eg. ">#") nor a slope into the exit, so omitting some checks
        if grid[x-1][y] not in ["#", "<", "v", ">", "O"]:
            # if test: print("EEP ^")
            if grid[x-1][y] == "^":
                # print(f"jumping ^ from {pos}")
                # n = dfs(grid, seen, (x-2, y))
                # dn=1 if n else 0
                stack.append((x-2,y,1,d+2))
            else:
                # n = dfs(grid, seen, (x-1, y))
                stack.append((x-1,y,1,d+1))
        if grid[x][y-1] not in ["#", "^", "v", ">", "O"]:
            # if test: print("EEP <")
            if grid[x][y-1] == "<":
                # print(f"jumping < from {pos}")
                # w = dfs(grid, seen, (x, y-2))
                # dw=1 if w else 0
                stack.append((x,y-2,1,d+2))
            else:
                # w = dfs(grid, seen, (x, y-1))
