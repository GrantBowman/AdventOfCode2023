...
# with open("input22_demo.txt") as input:
    ...
    for b1 in inputBrickList:
        if len(b1.supporting) == 0:
            # print(f"safe to disintegrate {b1} (lonely)")
            result +=1
            continue
        # need everything its supporting to still be supported
        safe = True
        for b2 in b1.supporting:
            if len(b2.supportedBy) == 1:
                safe = False
                # print(f"unsafe to disintegrate {b1} (atlas)")
                break
        if safe:
            result += 1
            # print(f"safe to disintegrate {b1} (helped)")
